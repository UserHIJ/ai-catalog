...
# ---------------- TRINO RUNNERS ----------------
run_sql_file() {
  # Write SQL to a temp file and execute via Trino CLI (--file) to avoid quoting bugs.
  local sql="$1"
...
clean_card() {
  local id="$1"
  echo "== [$id] CLEAN: drop raw table + delete catalog rows"
  run_sql_file "DROP TABLE IF EXISTS iceberg.datasets.${id};" >/dev/null || true
  run_sql_file "DELETE FROM iceberg.catalog.catalog_columns WHERE dataset_id='${id}';" >/dev/null || true
  run_sql_file "DELETE FROM iceberg.catalog.catalog_datasets WHERE dataset_id='${id}';" >/dev/null || true
  run_sql_file "DELETE FROM iceberg.catalog.catalog_lineage_edges WHERE dst_dataset_id='${id}' OR src_dataset_id='${id}';" >/dev/null || true
}
...
create_card() {
  ...
  echo "== [$id] 2) INSERT catalog_datasets"
  run_sql_file "
...
" | sed -n 's/^/   /p'
  ...
  # hard reset to prevent dup rows for same (dataset_id,column_name)
  run_sql_file "DELETE FROM iceberg.catalog.catalog_columns WHERE dataset_id='${id}';" >/dev/null || true
...
# ---------------- RUN: CLEAN & BUILD ----------------
# List of cards to rebuild fresh
cards=(
  "salesforce_opportunities_1|Salesforce Opportunities|Salesforce|salesforce_opportunities_raw|$DDL_SALESFORCE_OPPS_1"
  "shopify_orders_1|Shopify Orders|Shopify|shopify_orders_raw|$DDL_SHOPIFY_ORDERS_1"
  "zendesk_tickets_1|Zendesk Tickets|Zendesk|zendesk_tickets_raw|$DDL_ZENDESK_TICKETS_1"
  "jira_issues_1|Jira Issues|Jira Cloud|jira_issues_raw|$DDL_JIRA_ISSUES_1"
  "github_issues_1|GitHub Issues|GitHub|github_issues_raw|$DDL_GITHUB_ISSUES_1"
  "servicenow_incidents_1|ServiceNow Incidents|ServiceNow|servicenow_incidents_raw|$DDL_SERVICENOW_INCIDENTS_1"
  "netsuite_transactions_1|NetSuite Transactions|NetSuite|netsuite_transactions_raw|$DDL_NETSUITE_TXNS_1"
  "postgres_customers_1|PostgreSQL Customers|PostgreSQL|postgres_customers_raw|$DDL_POSTGRES_CUSTOMERS_1"
  "postgres_invoices_1|PostgreSQL Invoices|PostgreSQL|postgres_invoices_raw|$DDL_POSTGRES_INVOICES_1"
  "googleanalytics_events_1|Google Analytics Events|Google Analytics|googleanalytics_events_raw|$DDL_GA_EVENTS_1"
  "workday_positions_1|Workday Positions|Workday HCM|workday_positions_raw|$DDL_WORKDAY_POSITIONS_1"
  "anaplan_modelexports_1|Anaplan Model Exports|Anaplan|anaplan_modelexports_raw|$DDL_ANAPLAN_MODELEXPORTS_1"
  "oracleebs_gljournals_1|Oracle EBS GL Journals|Oracle EBS|oracleebs_gljournals_raw|$DDL_ORACLE_EBS_GLJOURNALS_1"
  "sapecc_materialmaster_1|SAP ECC Material Master|SAP ECC|sapecc_materialmaster_raw|$DDL_SAP_ECC_MATERIALMASTER_1"
  "sqlserver_clickstream_1|SQL Server Clickstream|SQL Server|sqlserver_clickstream_raw|$DDL_SQLSERVER_CLICKSTREAM_1"
  "github_releases_1|GitHub Releases|GitHub|github_releases_raw|$DDL_GITHUB_RELEASES_1"
  "zendesk_users_1|Zendesk Users|Zendesk|zendesk_users_raw|$DDL_ZENDESK_USERS_1"
  "shopify_customers_1|Shopify Customers|Shopify|shopify_customers_raw|$DDL_SHOPIFY_CUSTOMERS_1"
  "servicenow_changes_1|ServiceNow Changes|ServiceNow|servicenow_changes_raw|$DDL_SERVICENOW_CHANGES_1"
  "workday_compensation_1|Workday Compensation|Workday HCM|workday_compensation_raw|$DDL_WORKDAY_COMPENSATION_1"
)

for entry in "${cards[@]}"; do
  IFS='|' read -r id name source lineage ddl <<<"$entry"
  clean_card "$id"
  create_card "$id" "$name" "$source" "$lineage" "$ddl"
done
